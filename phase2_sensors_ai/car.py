...
# Add this method inside the Car class (after handle_input):

    def apply_ai(self, steer, accelerate, brake):
        """
        Apply AI decisions to the car's physics.
        Called instead of handle_input() when in AI mode.
        
        steer      : float (-ve = left, +ve = right)
        accelerate : bool
        brake      : bool
        """
        if accelerate:
            self.speed = min(self.speed + ACCELERATION, AI_TARGET_SPEED)
        if brake:
            self.speed = max(self.speed - BRAKE_FORCE, 0)
        if not accelerate and not brake:
            self.speed = max(self.speed - DECELERATION, 0)

        # Apply steering (only effective when moving)
        if abs(self.speed) > 0.5:
            self.angle += steer
